...
class PmChatsPage extends StatefulWidget {
  final int userId;
  final String userName;

  //use

  const PmChatsPage({super.key, required this.userId, required this.userName});

  @override
  State<PmChatsPage> createState() => _PmChatsPageState();
}

class _PmChatsPageState extends State<PmChatsPage> {
  ...
  Future<void> fetchChats() async {
    final url = Uri.parse(
        'http://$apiHost/remoco_app/api/get_pm_chats.php?pm_id=${widget.userId}');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(currentChatTitle ?? 'Project Manager Chats'),
        ...
        children: [
          Expanded(
            child: messages.isEmpty
              ...
              itemBuilder: (_, idx) => buildMessage(messages[idx],
                  key: ValueKey(messages[idx]['timestamp'])),
            ),
          ),
          Container(
            padding:
