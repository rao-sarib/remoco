<?php
// This page is served both directly (via AJAX) and as an include from
// tl_dashboard.php, which has already started the session. Starting it
// unconditionally raised a notice in the include case.
...
// Get team lead ID from session
$team_lead_id = $_SESSION['employee_id'];
...
try {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM tasks WHERE team_lead_id = ? AND task_status = 'In Progress'");
    $stmt->execute([$team_lead_id]);
    $task_stats['in_progress'] = $stmt->fetchColumn();

    // Get pending tasks (not started)
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM tasks WHERE team_lead_id = ? AND task_status = 'Not Started'");
    $stmt->execute([$team_lead_id]);
    $task_stats['pending'] = $stmt->fetchColumn();

    // Get completed tasks
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM tasks WHERE team_lead_id = ? AND task_status = 'Completed'");
    $stmt->execute([$team_lead_id]);
    $task_stats['completed'] = $stmt->fetchColumn();
...
<div class="tl-dashboard-container">
    ...
    <div class="recent-tasks">
        ...
        <div class="tasks-list">
            ...
            try {
                $stmt = $pdo->prepare("
                    SELECT task_id, title, task_status, due_date 
                    FROM tasks 
                    WHERE team_lead_id = ? 
